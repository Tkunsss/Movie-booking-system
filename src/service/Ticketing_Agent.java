...
public class Ticketing_Agent implements istaff{
    ...
    @Override
    // Check if agent can do an action.
    public boolean can(String action) {
        if (!active || isBlank(action)) return false;

        String permission = action.trim().toLowerCase();
        return permission.equals("create_menu_item")
                || permission.equals("view_orders")
                || permission.equals("update_order_status");
    }

    // ====== Constructor ======
    // Make an agent with data.
    public Ticketing_Agent(String staffId, String fullName, String phone,
