...
public class ManagerStaff implements istaff{
    ...
    @Override
    // Decide if this manager can do an action.
    public boolean can(String action) {
        if (!active || isBlank(action)) return false;

        String role = isBlank(position) ? "staff" : position.trim().toLowerCase();
        String permission = action.trim().toLowerCase();

        // Managers/Admins can do all actions except customer-only actions.
        if (role.equals("manager") || role.equals("admin")) {
            return !permission.equals("book_ticket")
                    && !permission.equals("cancel_booking")
                    && !permission.equals("view_booking");
        }
