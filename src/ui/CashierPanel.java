...
public class CashierPanel extends JPanel {
    ...
    private JPanel buildOrdersTab() {
        ...
        checkout.addActionListener(e -> checkoutOrder(list.getSelectedIndex()));
    ...
    private Customer promptCustomer() {
        ...
        try {
            ...
            return new Customer(id.getText().trim(), name.getText().trim(),
    ...
    private void createOrder() {
        if (state.customers.isEmpty() || state.movies.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Need customers and movies.");
            return;
        }
        String[] customerChoices = state.customers.stream()
        ...
        Customer customer = state.customers.get(indexOf(customerChoices, customerSel));
    ...
    private void checkoutOrder(int index) {
        ...
        if (!order.customer.deductBalance(order.totals.total)) {
        ...
        ReceiptGenerator.generateReceipt(order.customer, order.cart,
                order.totals.subtotal, order.totals.discount, order.totals.tax, order.totals.total);
        order.status = "PAID";
        state.orderDao.updateStatus(order.id, order.status);
        refreshOrders();
    }
